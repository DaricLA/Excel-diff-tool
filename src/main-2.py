...
class OpenpyxlComparer:
            ...
            for rule,o_sheet,o_addr,n_sheet,n_addr in shift_hits:
                ...
                for check in rule.checks:
                    ...
                    if ct in ('fill','font','border','number_format','row_height','col_width','alignment') and d.get('com_style'):
                        ...
                        diff=self._compare_by_check_type(ct,old_cell,new_cell,check.options,old_ws,new_ws,o_addr,o_sheet,new_address=n_addr,new_sheet_name=n_sheet,cf_ignore_range=True)
    ...
    def _display_equivalent(self,old_cell,new_cell,old_sheet,new_sheet,old_val=None,new_val=None):
        ...
        # 规则：按【双方数值小数位数的较少者】N 把双方 round N 位再比（截成一样长，保守取小）；
    ...
    def _compare_conditional_formats(self,old_ws,new_ws,sheet_name):
        seen_cf=set()  # 按 地址+描述 去重：同一 sqref 被重复解析时避免重复 diff
        ...
        for addr in sorted(set(addr_to_old)|set(addr_to_new)):
            ...
            if not old_items and new_items:
                ...
                for rng,cf in new_items:
                    ...
                    if (addr,desc) in seen_cf: continue
                    seen_cf.add((addr,desc))
    ...
    def _compare_by_check_type(self, check_type, old_cell, new_cell, options=None, old_ws=None, new_ws=None, address=None, sheet_name='', new_address=None, new_sheet_name='', cf_ignore_range=False):
        ...
        if check_type=='value':
            ...
            if cf_ignore_range:
                # shift 配对：条件格式按单元格独立比规则内容（类型/阈值/停止/格式），范围平移不算差异；
                # 被新扩展的单元格自身有 diff 时会自报"条件格式新增"，原单元格无需报范围变化
